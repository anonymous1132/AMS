       ...
       public List<ReqRpt018Model> ReqRpt018Models = new List<ReqRpt018Model>();
        ...
        private void GetModelList()
        {
            ...
            for (int i = 0; i < CountDay; i++)
            {
                ...
                {
                    l.Start_Time = l.Start_Time > firstTime.AddDays(i) ? l.Start_Time : firstTime.AddDays(i);
                    l.End_Time = l.End_Time < firstTime.AddDays(i + 1) ? l.End_Time : firstTime.AddDays(i + 1);
                }
                var list2= list.Select(s => new { s.EQP_ID, s.E10_State, DeltaTime= (s.End_Time - s.End_Time).TotalMinutes }).GroupBy(a => new { a.EQP_ID, a.E10_State }).Select(g=>new {Data=g.Key,AllSpan=g.Sum(item=>item.DeltaTime) });
