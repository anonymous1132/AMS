...
{
    public class InlineTempDebug:Inline
    {
       ...
       public override void GetData(string filePath,string SpecFile)
       {
            ...
            try
            {
                sql = string.Format("select * from fvace_inline_data where claim_time >= to_date('{0}','yyyy/mm/dd HH24:mi:ss')", GetLastDBLine());
            }
            catch (Exception)
            {
                sql = "select * from fvace_inline_data where lot='AE00037.02'";
            }
            //Test用
        ...
        public override string WriteSiff(string filePath)
        {
            ...
            using (FileStream fs = new FileStream(filepath, FileMode.Append, FileAccess.Write))
            {
                ...
                foreach (Inline_SigleLine inline in Inline_lines)
                {
                    try
                    {
                        List<string> sifflinelist = inline.GetSiffLines();
                        foreach (string str in sifflinelist)
                        {
                            sw.WriteLine("\""+DataSource+"\",\""+FormatVersion+"\","+str);
                        }
                    }
                    catch (Exception e)
                    {
