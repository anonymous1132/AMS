...
namespace LogParser
{
    public class ImapLogFileReader : LogFileReader<SmtpSslLogEntity, SmtpSslLineEntity>
    {
        public ImapLogFileReader(string filepath) : base(filepath)
        {
        ...
        public override void PutDB()
        {
            ...
            foreach (var entity in LogObj.LogEntities)
            {
                ...
                if (db.CountQuery<SmtpSslLogEntity>(Public.Config.ImapTop, filter) == 0)
                {
                    db.InsertOne<SmtpSslLogEntity>(Public.Config.ImapTop, entity);
                }
            }
        }
        ...
        private void SetSingleModelByLineList(List<SmtpSslLineEntity> linelist)
        {
            ...
            var reserve = linelist.Where(p => p.Command.Contains("用户名="));
            SmtpSslLogEntity smtpModel = new SmtpSslLogEntity
            {
                ...
                SendAddress = "",
                ReserveAddress = reserve.Any() ? reserve.First().Command.Split('=')[1] : "",
                MaxSize = 0,
