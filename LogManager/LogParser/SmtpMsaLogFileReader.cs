...
namespace LogParser
{
    public class SmtpMsaLogFileReader : LogFileReader<SmtpSslLogEntity, SmtpSslLineEntity>
    {
        public SmtpMsaLogFileReader(string filepath) : base(filepath)
        {
        }
        ...
        public override void PutDB()
        {
            ...
            foreach (var entity in LogObj.LogEntities)
            {
                ...
                if (db.CountQuery<SmtpSslLogEntity>(Public.Config.SmtpMsaTop, filter) == 0)
                {
                    db.InsertOne<SmtpSslLogEntity>(Public.Config.SmtpMsaTop, entity);
                }
            }
        }
