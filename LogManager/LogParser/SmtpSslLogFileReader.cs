...
namespace LogParser
{
    public class SmtpSslLogFileReader : LogFileReader<SmtpSslLogEntity, SmtpSslLineEntity>
    {
        public SmtpSslLogFileReader(string filepath) : base(filepath)
        {
        }
        ...
        public override void PutDB()
        {
            ...
            foreach (var entity in LogObj.LogEntities)
            {
                ...
                if (db.CountQuery<SmtpSslLogEntity>(Public.Config.SmtpSslTop, filter) == 0)
                {
                    db.InsertOne<SmtpSslLogEntity>(Public.Config.SmtpSslTop, entity);
                }
            }
        }
